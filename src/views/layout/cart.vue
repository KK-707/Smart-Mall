        ...
        <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
          ...
          <div class="info">
            ...
            <span class="bottom">
              ...
              <!-- 既想保留原本形参，又想通过调用函数传参，使用箭头函数包装一层 -->
              <CountBox
...
export default {
  ...
  computed: {
    // 映射 cart 模块中的 cartList 状态
    // 这样，在组件中就可以直接通过 this.cartList 来访问 cartList 状态
  ...
  created() {
    if (this.isLogin) {
      // 只有已经登录的才能调用方法获取数据
      this.$store.dispatch('cart/getCartAction') // 调用 store 里的 action 方法
    }
  },
  methods: {
    toggleCheck(goodsId) {
